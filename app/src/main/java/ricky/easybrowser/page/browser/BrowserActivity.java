...
public class BrowserActivity extends AppCompatActivity implements IWebView.OnWebInteractListener,
    ...
    protected void onRestoreInstanceState(Bundle savedInstanceState) {
        ...
        // Khi xoay màn hình, đẩy lại Fragment đã phục hồi vào bộ nhớ cache
        // Do danh sách tab có thể vượt quá kích thước của bộ nhớ cache (nghĩa là số lượng Fragment trước khi Activity bị hủy), ở đây trước tiên phục hồi thông tin tablist
        ...
        if (restoredFragmentList.size() > 0) {
            for (Fragment target : restoredFragmentList) {
                if (target instanceof ITab && target.getArguments() != null) {
                    // Dựa trên tham số Fragment, phục hồi thông tin TabInfo để tìm kiếm trong danh sách
